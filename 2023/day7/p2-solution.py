...
def handle_jokers(hand, num_each_card):
    if len(num_each_card) == 1 or "J" not in num_each_card:
        return
    
    max_card = ""
    max_card_count = 0
    for card in num_each_card:
        if card != "J" and num_each_card[card] > max_card_count:
            max_card = card
            max_card_count = num_each_card[card]

    num_jokers = hand.count("J")
    num_each_card[max_card] += num_jokers
...
def get_hand_rank(hand):
    ...
    handle_jokers(hand, num_each_card)
